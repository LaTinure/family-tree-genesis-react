    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('create-dynasty', {
        body: {
          createToken,
          dynastyData: {
            name: formData.dynastyName.trim(),
            description: formData.dynastyDescription.trim(),
          },
        },
      });

      if (error) {
        console.error('🚨 Échec backend:', error);
        throw new Error(error.message || 'Erreur lors de la création de la dynastie');
      }

      if (!data?.success) {
        throw new Error(data?.error || 'Erreur lors de la création de la dynastie');
      }

      console.log('✅ Dynastie créée:', data);
